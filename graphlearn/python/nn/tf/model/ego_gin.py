...
class EgoGIN(Module):
  ...
  Args:
    layers: A list, each element is an `EgoGINLayerGroup`.
  ...
  def forward(self, graph):
    """ Compute node embeddings through the GIN layers.
...
class HomoEgoGIN(EgoGIN):
  def __init__(self,
    ...
    for i in range(len(dims) - 1):
      ...
      group = EgoGINLayerGroup([layer] * (len(dims) - 1 - i))
      layers.append(group)

    super(HomoEgoGIN, self).__init__(layers, bn_fn, active_fn, dropout)
