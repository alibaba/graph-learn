...
class EgoGATLayerTestCase(unittest.TestCase):
  @classmethod
  def setUpClass(cls):
    pass
  ...
  def test_homogeneous_graph(self):
    # MAKE LAYERS
    dims = np.array([32, 8, 4])
    layer1 = EgoGATLayer("homo_1",
                          input_dim=dims[0],
                          output_dim=dims[1],
                          num_head=2)
    layer2 = EgoGATLayer("homo_2",
                          input_dim=dims[1],
                          output_dim=dims[2],
                          num_head=1)
    layer_group_1 = EgoGATLayerGroup([layer1, layer1])
    layer_group_2 = EgoGATLayerGroup([layer2])
  ...
  def test_heterogeneous_graph(self):
    ...
    layer_uv = EgoGATLayer("heter_uv",
                            input_dim=(dims_0[0], dims_0[1]),
                            output_dim=dims_1[0],
                            num_head=2)
    layer_vi = EgoGATLayer("heter_vi",
                            input_dim=(dims_0[1], dims_0[2]),
                            output_dim=dims_1[1],
                            num_head=2)
    layer_uvi = EgoGATLayer("heter_uvi",
                             input_dim=(dims_1[0], dims_1[1]),
                             output_dim=dims_2[0],
                             num_head=2)
    layer_group_1 = EgoGATLayerGroup([layer_uv, layer_vi])
    layer_group_2 = EgoGATLayerGroup([layer_uvi])
