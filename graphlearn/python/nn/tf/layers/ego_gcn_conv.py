...
# =============================================================================
"""EgoGraph based GCN convolutional layer"""
...
class EgoGCNConv(EgoConv):
  """ EgoGraph based implementation of GCN. https://arxiv.org/abs/1609.02907.
  ...
  def __init__(self,
               ...
               use_bias=False,
               **kwargs):
    super(EgoGCNConv, self).__init__()
    ...
    with tf.variable_scope("ego_gcn_layer_" + name, reuse=tf.AUTO_REUSE):
      ...
      if num_bases is not None:
        ...
        self.weight = tf.get_variable(
          name='weight',
          shape=[num_blocks, self._in_dim[1] // num_blocks, out_dim // num_blocks])
      else:
        self.weight = tf.get_variable(
          name="weight", 
          shape=[self._in_dim[1], self._out_dim])
  ...
  def forward(self, x, neighbor, expand):
    ...
    if self._num_bases is not None:
      ...
      weight = tf.reshape(tf.matmul(self.coefficient, weight), 
                          [self._in_dim[1], self._out_dim])

    if self._num_blocks is not None:
      h = agg_func(neighbor, expand, self._in_dim[1])
      h = tf.reshape(h, [-1, self._num_blocks, self._in_dim[1] // self._num_blocks])
      h = tf.einsum('aij,ijk->aik', h, weight)
      out = tf.reshape(h, [-1, self._out_dim])
    else:
      h = agg_func(neighbor, expand, self._in_dim[1])
