...
class _SamplerType(Enum):  # pylint: disable=invalid-name
  """ The action on the current path, including sample and get value from
...
class EdgeQuery(Query):
  ...
  def __init__(self, query_engine, edge_type, feed=None, **kwargs):
...
class QueryEngine(object):
  ...
  def parse_tree(self, query_root, func):
    ...
    queue = [query_root]
    parsed_root = _ParsedNode(_PositionedSampler(
      _NamedSource(source, query_root.name), cur_query.traversal_path))
    ...
    while queue:
      ...
      for child in node.children:
        ...
        if child_value.sampler_path:
          ...
          s = None
        parsed = _ParsedNode(_PositionedSampler(
          _NamedSource(s, child.name), child_value.traversal_path))
