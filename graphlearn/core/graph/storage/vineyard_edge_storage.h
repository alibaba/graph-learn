...
public:
  explicit VineyardEdgeStorage(std::string edge_label = "0",
    ...
    if (src_label_index == vlabels.end()) {
      throw std::runtime_error(
          "Edge: failed to find src node label in local fragment: " + src_node_type);
    } else {
    ...
    if (dst_label_index == vlabels.end()) {
      throw std::runtime_error(
          "Edge: failed to find dst node label in local fragment: " + dst_node_type);
    } else {
    ...
    side_info_ = frag_edge_side_info(frag_, attrs_, edge_label_name,
                                     src_node_type, dst_node_type, edge_label_);
  }
