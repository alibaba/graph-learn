...
public:
  ...
  Status Process(const OpRequest* req,
    ...
    if (request->IsDeepWalk()) {
      ...
      if (walk_len <= 1) {
        response->AppendWalks(ret_walks.data(), total_size);
        response->SetSparseFlag();
        response->InitNeighborIds(total_neighbor_count);
        response->InitDegrees(batch_size);
      ...
      for (int32_t step = 1; step < walk_len; ++step) {
        ...
        auto begin = ret_walks.begin() + batch_size * std::max(step - 2, 0);
...
void DeepWalk(const RandomWalkRequest* req,
  ...
  void WeightedRandomWalk(const RandomWalkRequest* req,
    ...
    auto parent_neighbor_indices = req->GetParentNeighborIndices();
    ...
    for (int32_t i = 0; i < batch_size; ++i) {
      ...
      if (nbr_count == 0) {
        ...
        auto indice = WeightedRandomWalkKernel(
          ...
          cursor, parent_neighbor_indices[i],
