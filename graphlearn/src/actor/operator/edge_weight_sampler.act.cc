...
#include "actor/operator/edge_weight_sampler.act.h"
...
EdgeWeightSamplerActor::EdgeWeightSamplerActor(brane::actor_base *exec_ctx,
    const brane::byte_t *addr, const void* params)
    : StatelessBaseOperatorActor(exec_ctx, addr, "EdgeWeightSampler") {
...
EdgeWeightSamplerActor::~EdgeWeightSamplerActor() {}

seastar::future<TensorMap>
EdgeWeightSamplerActor::Process(TensorMap&& tensors) {
  // create request
...
OpRefRegistration<EdgeWeightSamplerActorRef>
  _EdgeWeightSamplerActorRef("EdgeWeightSampler");
