...
#include "actor/operator/in_degree_sampler.act.h"
...
InDegreeSamplerActor::InDegreeSamplerActor(brane::actor_base *exec_ctx,
    const brane::byte_t *addr, const void* params)
    : StatelessBaseOperatorActor(exec_ctx, addr, "InDegreeSampler") {
...
InDegreeSamplerActor::~InDegreeSamplerActor() {}

seastar::future<TensorMap> InDegreeSamplerActor::Process(TensorMap&& tensors) {
  // create request
...
OpRefRegistration<InDegreeSamplerActorRef> _InDegreeSamplerActorRef(
  "InDegreeSampler");
