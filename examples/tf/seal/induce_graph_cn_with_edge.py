...
def induce_graph_cn_with_edge(src_nodes, dst_nodes, 
  src_nbrs, dst_nbrs):
  ...
  for i in range (src_nodes.ids.size):
    ...
    row, col, struct_label = gen_edge_index_cn(i, src_nbrs, dst_nbrs,
        src_offset, dst_offset, col_offset,
        row, col, struct_label, src=True)
    col_offset += src_nbrs.offsets[i]
    row, col, struct_label = gen_edge_index_cn(i, dst_nbrs, src_nbrs,
...
def gen_edge_index_cn(idx, src_nbrs, dst_nbrs,
                           src_offset, dst_offset, col_offset,
                           row, col, struct_label, src=True):
  ...
  for j in range(src_nbrs.offsets[idx]):
    ...
    if (src_nbrs.ids[src_offset+j] in
      ...
      struct_label.append(1)
    else:
      struct_label.append(2)
