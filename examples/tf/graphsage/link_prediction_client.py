...
def dist_training(handle, config):
  ...
  # ]
  config['gl_hosts'] = [
    "{}:{}".format(pod_name, 9000 + index) for index, pod_name in enumerate(hosts[0:])
  ]
...
def main():
  ...
  handle['node_schema'] = ['paper:false:false:100:0:0']
  gl.set_attr_start_index(0)
  gl.set_attr_end_index(100)
